...
t1.shape("turtle")  # поменяли фоrandint(0,255)рму объекта
t1.color(111, 111, 111)    # меняем цвет объекта
...
t2.pendown()  # опускаем черепашку чтоб потом
t2.speed(3)
...
# здесь я прикрепляю обработчик к событию нажатия на 1 черепашку
t1.onclick(catch1)


def catch2(x, y):   # это обработчик события нажатия
    t2.write('РњРЅРµ Р±РѕР»СЊРЅРѕ!',  font=(
        'Arial', 14, 'normal'))    # пишем на экране Ауч
...
while t1.xcor() < finish and t2.xcor() < finish:
    # здесь черепеаха двигается вперед и рисует тоже на случайное число в диапазоне от 2 о 7
