...
class Chromosome:
    ...
    '''
    Теперь напишем функцию для генерации случайной хромосомы. Функция принимает два параметра: длина хромосомы, 
    которую надо получить, и набор генов, из которого нужно сделать хромосому.
    '''
...
def create_population(pop_size, chromo_size, genes):
    '''
    В функцию заполнения популяции мы также передаем размер популяции, размер хромосомы и генофонд, чтобы не 
    зависеть от глобальных переменных. Подбор пар.
    '''
    ...
    return population
'''
функция для вычисления рейтинга - расстояния между двумя строками. Напишем сразу для всей популяции, т.к. других
применений у нее нет.
'''
    ...
    for chromo in population:
        ...
        for i in range(chromo.size):
            if chromo.genes[i] == final_chromo[i]:
                chromo.rating -= 1
'''
сделаем сортировку хромосом по рейтингу. Это обычная сортировка пузырком:
'''
...
def repopulate(population, parents, children_count):
    '''
    Теперь, имея функцию для выбора родителей и для скрещивания, пишем фунцию. которая заполняет вторую половину
    популяции потомками (родители сохраняются в перовй половине)
    '''
...
def cross(chromo1, chromo2):
    '''
    Получив двух родителей, мы можем провести скрещивание. Его суто в том, чтобы каким-то образом перемешать гены
    двух родителей и получить новую хромосому-потомка.
    ...
    и гены родителя №2 от этой позиии и до конца.
    
    Каждые два родителя порождают пару потомков. Т.е. функцию cross() мы вызываем дважды: сначала с (родитель1, 
    родитель2), затем с (родитель2, родитель 1).
    '''
    size = chromo1.size
...
def mutate(population, chromo_count, gene_count, gene_pool):
    '''
    Можно подвергать мутации хоть 50% популяции, но вот количество генов лучше задать 1. Это значит, что за 1 раз
    мутирует только один символ в строке. Были нередки случаи, когда строка была уже почти правильная. т.е.
    отличалась всего одним символом, но если после мутации в ней меняется больше чем один символ. мы наоборот
    удаляемся от цели.
    '''
    pop_size = len(population)
...
'''
Генофонд - это строка справочник, которая содержит все возможные гены. Его и финальную строку (будем теперь говорить
по-научному: хромосому) мы закодируем в байтовые массивы
'''
gene_pool = bytearray(b'abcdefghijklmnopqrstuvwxyz ') # если хотите по-русски, то ставьте сюда весь русский алфавит
...
population_size = 20
'''
В нашем случае мы для селекции возьмем меод элит. У нас будет элита в виде лучшей половины популяции, которая будет 
переходить в новую популяцию и порождать потомков, чтобы заполнить вторую полувину новой популяции.

Список хромосом уже отсортирован по рейтингу и поэтому задача селекции решена (берем топ-10), но ради будущей реализации
других методов нам надо сделать формальный отбор. То есть - поместить наших избранников в список "выживших".

Заведем для выживших список фиксированнной длины заранее и будем им пользоваться все время.
'''
...
while True:  
    ...
    if population[0].rating == 0:   
        '''
        При достижении целевой строки у первой хромосомы в списке будет рейтинг 0. Обнаружив такое условие, мы 
        прекращаем цикл, т.к. цель достигнута. Мы также печатаем текущую позицию на каждом шаге цикла:
        '''
        break
